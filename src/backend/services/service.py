...
class Service:
    def __init__(self, habit_repository: HabitRepository, user_repository: UserRepository, xp_provider, streak_service=None):
    ...
    def register_user(self, username: str, password: str) -> dict[str, str | int]:
        ...
        existing_id = self.user_repository.get_user_id_by_username(normalized)
    ...
    def login_user(self, username: str, password: str) -> dict[str, str | int]:
        ...
        user_id = self.user_repository.get_user_id_by_username(normalized)
        if user_id is None:
            raise ValueError("Invalid username or password.")

        stored_hash = self.user_repository.get_password_hash(user_id)
    ...
    def delete_habit(self, user_id: int, habit_id: int):
        ...
        if habit is not None:
            is_completed = self.habit_repository.get_current_period_completion(habit_id, habit.frequency)
            if is_completed:
                xp = self.xp_provider.resolve_xp(habit)
                self.user_repository.update_xp(user_id, -xp)
                self.user_repository.increment_completions(user_id, -1)
        self.habit_repository.delete_completions(habit_id)
    ...
    def toggle_habit_completion(self, user_id: int, habit_id: int) -> dict:
        ...
        if completed_habit is not None:
            if is_completed:
                xp_reward = self.xp_provider.resolve_xp(completed_habit)
                self.user_repository.update_xp(user_id, xp_reward)
                self.user_repository.increment_completions(user_id, 1)
                if self.streak_service:
                    ...
                    if streak_bonus_xp > 0:
                        self.user_repository.update_xp(user_id, streak_bonus_xp)
            else:
                xp_reward = self.xp_provider.resolve_xp(completed_habit)
                self.user_repository.update_xp(user_id, -xp_reward)
                self.user_repository.increment_completions(user_id, -1)

        progress = self.user_repository.get_progress(user_id)
    ...
    def get_habit_streaks(self, user_id: int) -> dict[int, int]:
        if not self.streak_service:
            return {}
        data = self.habit_repository.get_all_streaks(user_id)
